...
Original file is located at
    https://colab.research.google.com/drive/1R-VaVG0o3OEdZ0iAhYtoJgdn9DhFrOH_
...
def generate_gaussian(miu, sigma, num_samples):
  ...
  # MULAI KODE DI SINI
  uniform_vals = generate_rand_uniform(0, 1, num_samples)
  array = uniform_inverse_cdf(uniform_vals, miu, sigma)
  # Petunjuk:
...
def viz_distribusi_gaussian(first_gaussian, second_gaussian, third_gaussian):
  ...
  # MULAI KODE DI SINI
  ax.hist(first_gaussian, bins=32, alpha=0.5, label='Gaussian(0,1)')
  ax.hist(second_gaussian, bins=32, alpha=0.5, label='Gaussian(12,6)')
  ax.hist(third_gaussian, bins=32, alpha=0.5, label='Gaussian(10,5)')
  # Tugas:
...
def generate_binomial(n_trials, success_prob, num_samples):
  ...
  # MULAI KODE DI SINI
  uniform_vals = generate_rand_uniform(0, 1, num_samples)
  array = binomial_inverse_cdf(uniform_vals, n_trials, success_prob)
  # Petunjuk:
  # - Gunakan fungsi generate_rand_uniform untuk menghasilkan data acak uniform dengan rentang 0 dan 1.
...
def viz_distribusi_binomial(first_binomial, second_binomial, third_binomial):
  ...
  # MULAI KODE DI SINI
  ax.hist(first_binomial, alpha=0.5, label='Binomial(12, 0.4)')
  ax.hist(second_binomial, alpha=0.5, label='Binomial(15, 0.5)')
  ax.hist(third_binomial, alpha=0.5, label='Binomial(25, 0.8)')
  # Tugas:
  ...
  for feature in features:
    match feature:
      case 'wingspan_cm' | 'weight_g':
        df[feature] = generate_gaussian(params[breed][feature].miu, params[breed][feature].sigma, n_samples)
      case 'sing_days':
        df[feature] = generate_binomial(params[breed][feature].n_trials, params[breed][feature].probability, n_samples)
      case 'beak_head_ratio':
        df[feature] = generate_rand_uniform(params[breed][feature].lower_bound, params[breed][feature].upper_bound, n_samples)
...
def pmf_for_binomial(val, n_trials, probability):
  ...
  # MULAI KODE DI SINI
  pmf = binom.pmf(val, n=n_trials, p=probability)
  # AKHIRI KODE DI SINI
